    ...
    ImmutableDictionary<FlowAddress, Type>? narrowedTypes = null)
{
    ...
    public ImmutableDictionary<FlowAddress, Type> NarrowedTypes { get; private init; } = narrowedTypes ?? ImmutableDictionary<FlowAddress, Type>.Empty;

    // O(1): shares the same underlying trees as `from` rather than copying elements.
    ...
    public FlowState WithInitialized(Symbol symbol) => new(this)
    {
        DefinitelyInitialized = DefinitelyInitialized.Add(symbol),
        MaybeInitialized = MaybeInitialized.Add(symbol),
    };
    ...
    public FlowState WithInitialized(IEnumerable<Symbol> symbols)
    {
        ...
        {
            DefinitelyInitialized = definitelyBuilder.ToImmutable(),
            MaybeInitialized = maybeBuilder.ToImmutable(),
        };
    }

    public FlowState Merge(FlowState other) => new(
    ...
    private ImmutableDictionary<FlowAddress, Type> MergeNarrowedTypes(FlowState other)
    {
        ...
        foreach (var (address, type) in smaller)
        {
            if (larger.TryGetValue(address, out var otherType))
                builder[address] = TypeSimplifier.Simplify(new TypeChecking.Types.UnionType([type, otherType]));
        }
    ...
    public sealed class Builder
    {
        ...
        public FlowState ToImmutable() => new(
